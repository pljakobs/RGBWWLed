...
void RGBWWLed::setOutput(ChannelOutput& output)
{
	if(_pwm_output != NULL) {
		...
		_pwm_output->setOutput(getDimCurveValue(output.r), getDimCurveValue(output.g), getDimCurveValue(output.b),
							   getDimCurveValue(output.ww), getDimCurveValue(output.cw));
	}
};
...
bool RGBWWLed::fadeHSV(const RequestHSVCT& color, const RampTimeOrSpeed& ramp, int direction, bool requeue,
					   const String& name)
{
	return fadeHSV(color, ramp, direction, QueuePolicy::Single, requeue, name);
}

bool RGBWWLed::fadeHSV(const RequestHSVCT& color, const RampTimeOrSpeed& ramp, QueuePolicy queuePolicy, bool requeue,
					   const String& name)
{
	return fadeHSV(color, ramp, 1, queuePolicy, requeue, name);
}

bool RGBWWLed::fadeHSV(const RequestHSVCT& color, const RampTimeOrSpeed& ramp, int direction, QueuePolicy queuePolicy,
...
bool RGBWWLed::fadeHSV(const RequestHSVCT& colorFrom, const RequestHSVCT& color, const RampTimeOrSpeed& ramp,
					   int direction, QueuePolicy queuePolicy, bool requeue, const String& name)
{
	_mode = ColorMode::Hsv;
	...
	if(ramp.type == RampTimeOrSpeed::Type::Time && (ramp.value == 0.0 || ramp.value < RGBWW_MINTIMEDIFF)) {
		...
		result &= pushAnimTransitionCircularHue(colorFrom.h, color.h, ramp, direction, queuePolicy, CtrlChannel::Hue,
												requeue, name);
...
bool RGBWWLed::setRAW(const RequestChannelOutput& output, int time, QueuePolicy queuePolicy, bool requeue,
					  const String& name)
{
	_mode = ColorMode::Raw;
...
bool RGBWWLed::fadeRAW(const RequestChannelOutput& output, const RampTimeOrSpeed& ramp, QueuePolicy queuePolicy,
					   bool requeue, const String& name)
{
	_mode = ColorMode::Raw;
...
bool RGBWWLed::fadeRAW(const RequestChannelOutput& output_from, const RequestChannelOutput& output,
					   const RampTimeOrSpeed& ramp, QueuePolicy queuePolicy, bool requeue, const String& name)
{
	_mode = ColorMode::Raw;
...
bool RGBWWLed::pushAnimSetAndStay(const Optional<AbsOrRelValue>& val, int time, QueuePolicy queuePolicy, CtrlChannel ch,
								  bool requeue, const String& name)
{
	if(!val.hasValue())
		return true;
...
bool RGBWWLed::pushAnimTransition(const Optional<AbsOrRelValue>& val, const RampTimeOrSpeed& change,
...
bool RGBWWLed::pushAnimTransition(const AbsOrRelValue& from, const Optional<AbsOrRelValue>& val,
								  const RampTimeOrSpeed& change, QueuePolicy queuePolicy, CtrlChannel ch, bool requeue,
								  const String& name)
{
	if(!val.hasValue())
		return true;
...
bool RGBWWLed::pushAnimTransitionCircularHue(const Optional<AbsOrRelValue>& val, const RampTimeOrSpeed& change,
											 int direction, QueuePolicy queuePolicy, CtrlChannel ch, bool requeue,
											 const String& name)
{
	if(!val.hasValue()) {
		return true;
...
bool RGBWWLed::pushAnimTransitionCircularHue(const AbsOrRelValue& from, const Optional<AbsOrRelValue>& val,
											 const RampTimeOrSpeed& change, int direction, QueuePolicy queuePolicy,
...
bool RGBWWLed::dispatchAnimation(RGBWWLedAnimation* pAnim, CtrlChannel ch, QueuePolicy queuePolicy,
...
void RGBWWLed::callForChannels(const ChannelGroup& group, void (RGBWWAnimatedChannel::*fnc)(),
							   const ChannelList& channels)
{
